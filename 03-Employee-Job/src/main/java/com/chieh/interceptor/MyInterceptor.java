...
public class MyInterceptor implements HandlerInterceptor {
    ...
    /*
        ...
        返回值boolean:
         ...
         2.在这个方法中可以获取请求的信息, 验证请求是否符合要求.
            可以验证用户是否登录, 验证用户是否由权限访问某个连接地址(url).
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //除了登录页面和index其它都拦截判断是否已登录
        String path = request.getRequestURI();
        if(path.indexOf("login") != -1 || path.indexOf("index.jsp") != -1 ){
            System.out.println("访问的初始页面或登录,放行");
            return true;
        }
    ...
    /*
        ...
        特点:
         ...
         2.一般做资源回收工作的，程序请求过程中创建了一些对象，在这里可以输出， 把占用的内存回收。
     */
    @Override
